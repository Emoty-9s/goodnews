...
### REDIS_URL — 직접 입력 필수 (참조식 사용 불가)
`${{Redis.REDIS_URL}}` 참조식은 Railway UI 버그로 인해 `<empty string>`으로 치환되는 경우가 있다.
반드시 Redis 서비스 → Variables 탭에서 REDIS_URL 실제 값을 복사해 worker/beat에 직접 붙여넣는다.
worker, beat 두 서비스 모두 동일하게 적용.
...
| worker/beat Deploy Crashed — `amqp://127.0.0.1:5672` 연결 시도 | REDIS_URL 환경변수 누락 → Celery가 RabbitMQ 기본값으로 폴백 | worker/beat Variables에 REDIS_URL 직접 입력 |
| `REDIS_URL = ${{Redis.REDIS_URL}}` 입력 후 `<empty string>`으로 표시됨 | Railway UI 버그 — 참조식이 치환되지 않음 | Redis 서비스 Variables에서 실제 URL 복사 후 하드코딩 |
...
- [ ] ET 21:00 daily_closing 배치 첫 실행 로그 확인
- [ ] /summary/AAPL?digest_type=daily API 응답 확인 (배치 후)
