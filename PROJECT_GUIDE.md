...
> 최종 갱신: 2026-07-01
> (전체 파이프라인 실가동 완료 — worker/beat Redis 연결, 4,004개 종목 백필, daily-closing 첫 자동 실행)
...
| AI 요약 | Google Gemini — `gemini-2.5-flash-lite`(전체. daily/weekly/midterm/sector 모두 flash-lite 사용) |
| 구조화 출력 | Gemini `response_schema`(Pydantic) — daily/weekly/midterm 공통 적용 |
| 중복 제거 | Jaccard similarity (단어 집합 기준) |
| 스케줄러 | Celery + Redis (Railway 배포 + Redis 연결 확인 + 실가동 완료, 2026-07-01) |
...
## 3. 클라우드 인프라 구성 (2026-07-01 현재)

### Railway 프로젝트: "Good News" (production 환경)

| 서비스 이름 | 역할 | 상태 | Start Command |
|---|---|---|---|
| `API_MAIN` | FastAPI 웹 서버 | Online ✅ (/health 확인, 도메인 변경됨) | `sh -c "uvicorn app.api.main:app --host 0.0.0.0 --port $PORT"` |
| `worker` | Celery 워커 (배치 실행) | Online ✅ (Redis 연결 확인, daily-closing 실가동 완료) | `celery -A app.scheduler.tasks worker --loglevel=info` |
| `beat` | Celery 비트 (스케줄러) | Online ✅ (Redis 연결 확인, 스케줄 실가동 완료) | `celery -A app.scheduler.tasks beat --loglevel=info` |
| `Redis` | 메시지 브로커 | Online ✅ | (Railway 관리형) |

**공개 도메인**: `https://apimain-production-1503.up.railway.app`
...
| `REDIS_URL` | **직접 입력 필수** — `${{Redis.REDIS_URL}}` 참조식은 Railway UI 버그로 빈 문자열이 됨. Redis 서비스 Variables에서 실제 값 복사해서 하드코딩 |
| `APP_ENV` | `production` |
...
```
[1단계] 유니버스 빌드 (연 1회, 매년 1월 1일 03:00 ET — 거래소 휴장일)
  FMP /stable/company-screener
  → NASDAQ / NYSE / AMEX 상장, 시총 1억 USD 이상, ETF·워런트·우선주 등 제외
  → 4,004개 종목 → Supabase universe_tickers 테이블 (TRUNCATE + INSERT)
  → 로컬 CSV는 검토/디버깅용 사본 (source-of-truth는 항상 Supabase)
  → 수동 트리거: celery -A app.scheduler.tasks call tasks.build_universe

[2단계] 뉴스 수집
  [백필] backfill_full.py — 주차별 순환 처리 (수집→요약→articles 삭제 반복, 용량 최적화)
...
### universe_tickers (종목 유니버스 — 연 1회 갱신)
```sql
symbol               TEXT          PK
company_name         TEXT
exchange             TEXT
...
**갱신**: 매년 1월 1일 03:00 ET 자동 실행 (`tasks.build_universe`)
**용도**: daily/weekly/midterm 배치 대상 종목 목록 + 섹터/거래소 메타데이터
**현재 상태**: 2026-06-30 최초 업로드 완료, 4,004개 종목
...
> `universe_tickers`는 자동 삭제 대상 아님 — 연 1회 TRUNCATE + 전체 재삽입.
...
│       └── universe_save.py
├── scripts/
│   ├── backfill_full.py                     주차별 순환 전체 백필 (신규 — 권장 방식)
│   ├── fetch_news_backfill.py               FMP 뉴스 12주치 수집 → articles 업로드
│   ├── backfill_benchmarks_news.py          벤치마크 + 시장뉴스 + 거시지표 소급 수집
│   ├── backfill_summaries.py                요약 소급 생성 (Phase 1~4)
│   ├── run_backfill_universe.py             전체 유니버스 백필 래퍼 스크립트
...
**모델 분리**
- daily / weekly / midterm / sector: 전부 `gemini-2.5-flash-lite` (비용 절감)
- `gemini-2.5-flash`는 config에 정의만 되어있고 현재 미사용
...
운영 시작 전 또는 데이터 공백 발생 시 과거 데이터를 소급 채우는 스크립트.

### backfill_full.py — 주차별 순환 전체 백필 (권장)
```bash
python scripts/backfill_full.py                          # 전체 실행 (12주, universe 전체)
python scripts/backfill_full.py --weeks 4                # 최근 4주만
python scripts/backfill_full.py --tickers AAPL,NVDA      # 특정 종목만
python scripts/backfill_full.py --force                  # 로컬 캐시 무시, FMP 재수집
python scripts/backfill_full.py --skip-benchmarks        # 벤치마크 수집 건너뜀
python scripts/backfill_full.py --skip-daily             # daily overnight 생성 건너뜀
python scripts/backfill_full.py --from-week 2026-04-07   # 특정 주부터 재개
python scripts/backfill_full.py --concurrency 25         # Gemini 동시 호출 수
```

**주차별 순환 처리**: 1주치 FMP 수집 → articles 업로드 → weekly 요약 → articles 즉시 삭제를 12회 반복.
articles는 항상 1주치만 DB에 존재 → Supabase 500MB 제한 문제 없음.
Step 0(벤치마크), Step 1~12(주차 루프), Step 13(midterm), Step 14(daily) 구조.
4,004개 × 12주 기준 약 3~4시간 소요.

> 기존 `fetch_news_backfill.py` + `backfill_summaries.py` 두 스크립트를 별도 실행하는 방식은
> articles 용량 초과 및 캐시 오염 문제로 인해 `backfill_full.py`로 대체됨.
> 개별 Phase 재실행이나 디버깅 목적으로는 기존 스크립트도 계속 사용 가능.
...
| 유니버스 저장소 CSV → Supabase 전환 | Railway 컨테이너 디스크는 휘발성 + 서비스 간 공유 불가 → Supabase `universe_tickers` 테이블을 단일 소스로 통일 |
| 유니버스 빌드 주기 주 1회 → 연 1회 | 종목 유니버스는 자주 안 바뀜 → FMP API 호출/비용 절감, 매년 1월 1일 자동 실행 |
| REDIS_URL 직접 입력 채택 | `${{Redis.REDIS_URL}}` 참조식이 Railway UI에서 빈 문자열로 평가되는 버그 발견 → 하드코딩 필수 |
| backfill_full.py 주차별 순환 방식 도입 | 12주치 articles 한꺼번에 올리면 Supabase 500MB 초과 → 1주치씩 처리 후 즉시 삭제 |
| Gemini 모델 flash → flash-lite 전체 통일 | weekly/midterm/sector도 flash-lite로 충분한 품질 확인 → 전체 비용 절감 |
| Pydantic 스키마 default 값 제거 | Gemini response_schema에 default 값 있는 필드 허용 안 함 → 전체 제거 |
| PROMPT_FULL 중괄호 이스케이프 | JSON 예시({...})를 .format()에 넣으면 KeyError 발생 → `{{` / `}}` 이스케이프 처리 |

---

## 14. 현재 진행 상태 (2026-07-01 기준)
...
- [x] **Celery worker/beat Redis 연결 + 실가동 완료 (2026-07-01)**
  - REDIS_URL 환경변수 누락 → RabbitMQ 기본값 폴백 크래시 발견 및 해결
  - `${{Redis.REDIS_URL}}` 참조식 Railway UI 버그 → 직접 입력으로 해결
  - worker / beat 모두 Redis 정상 연결, `celery@... ready.` 확인
  - daily-closing 첫 자동 실행 성공 (ET 21:00, 2026-06-30)
- [x] **버그 수정 5종 (2026-07-01)**
  - pandas import 최상단 → 함수 내부 이동 (Railway worker 크래시 해결)
  - PROMPT_FULL KeyError: `'"content"'` → `{{` / `}}` 이스케이프 처리
  - Pydantic 스키마 default 값 → Gemini API 호환 오류 전체 수정
  - `backfill_summaries.py` Phase 3 — 7일 skip 제거, 거시지표 macro_data 주입
  - midterm report_date 및 벤치마크 기간 → 전체 12주 기준으로 고정 (종목마다 제각각이던 문제 해결)
- [x] **유니버스 저장소 CSV → Supabase 전환 (2026-07-01)**
  - `universe_tickers` 테이블 신규 생성 (init_db.sql + Supabase 적용)
  - `ticker_store.py` 3개 함수 async 전환, DB 조회 방식으로 변경
  - `task_build_universe` → Supabase에 직접 upsert하도록 수정
  - beat_schedule: universe-weekly(매주 일요일) → universe-yearly(매년 1월 1일 03:00 ET)
  - 로컬 CSV ↔ Supabase 양방향 스크립트 2종 신규 작성
  - 4,004개 종목 최초 업로드 완료 (2026-06-30)
- [x] **backfill_full.py 주차별 순환 백필 신규 작성 (2026-07-01)**
  - 기존 2-스크립트 방식(articles 용량 초과 문제) 대체
  - 1주치 FMP 수집 → articles 업로드 → weekly 요약 → articles 즉시 삭제 × 12회 반복
  - `--force`, `--from-week`, `--skip-benchmarks` 등 옵션 완비
- [x] **4,004개 전체 유니버스 12주 백필 완료 (2026-07-01)**
  - weekly/final: 20,008건 생성
  - midterm/final: 3,764건 생성 (240개 종목은 FMP 뉴스 없어 스킵)
  - API 응답 확인: /summary/NVDA, /summary/AAPL, /summary/TSLA 모두 정상
- [x] **API 정상 서빙 확인 (2026-07-01)**
  - 공개 도메인: `https://apimain-production-1503.up.railway.app`
  - weekly, midterm 200 응답 확인
  - daily는 오늘 밤 ET 21:00 배치 후 채워질 예정

### 다음 단계
- [ ] **daily-closing 안정적 자동 실행 모니터링**
  - 매일 ET 21:00 자동 실행 확인 (첫 자동 실행: 2026-06-30 성공)
  - 실패율 5% 이하 유지 여부 모니터링
- [ ] **프론트엔드 개발**
  - 종목 검색 + 리포트 조회 UI
  - daily / weekly / midterm 탭 전환
- [ ] **사용자 관심종목 등록/관리 API**
- [ ] **요약 품질 A/B 테스트**
